...
| Status Brief Builder | Turn meeting notes into a brief | C.A.R.E. | Context: Raw notes from an EAS pilot check-in. Action: Summarize into a decision-ready brief. Rules: Keep names/dates/numbers; don't convert suggestions into decisions. Expected Output: 3-bullet summary + action table (Owner, Deadline, Priority, Status). | Short brief with clear actions |
| Delay Notice Email | Communicate a schedule slip | R.C.T.O. | Role: Business Analyst. Context: EAS pilot delayed from Oct 1 to Oct 10 due to vendor testing. Task: Write a short update email to stakeholders. Output: Subject + 2 short paragraphs, professional tone, no blame. | Concise, accurate delay email |
...
| Rollout Plan Chain | Build a phased rollout plan | C.A.R.E. | Context: 4-week EAS pilot for one department. Action: Break goal into mechanisms, phases, tasks. Rules: Keep realistic; no invented owners/dates. Expected Output: Goal → Mechanisms → Phases → Tasks. | Short, actionable plan |
